...
public class ItemServiceIntegrationTest extends BaseFullStackTest {
    ...
    void shouldFetchTwoDifferentEntitiesSameItemId() {
        ...
        verify(itemFetcher).fetchItem(eq(itemId)); // the second time was pulled from the cache
        verify(blizzardServiceClient, times(1)).getBaseItem(itemId); // should pull from cache the second time
